...
interfaceUtils.setBufferLimit(1024)
...
Start_X, starty, Start_Z, End_X, endy, End_Z = interfaceUtils.requestBuildArea()
    ...
    for y in range(y1 + 1, y2):
        for x in range(x1 + 1, x2 - 1):
            for z in range(z1 + 1, z2 - 1):
                ...
                print(interfaceUtils.getBlock(x, y, z))
                # print(type(interfaceUtils.getBlock(x, y, z)))
                # if "water" in interfaceUtils.getBlock(x, y, z):
                #     print('-->water')
                if "minecraft:water" == interfaceUtils.getBlock(x, y, z):
                     print('-->2 water')
...
if __name__ == '__main__':
    ...
    print(f"Build area is at position {Start_X}, {Start_Z}"
          f" with size {End_X-Start_X+1}, {End_Z-Start_Z+1}")

    # load the world data
    # this uses the /chunks endpoint in the background
    worldSlice = WorldSlice(Start_X, Start_Z, End_X, End_Z)
    ...
    for i in range(100):
        ...
        houseX = random.randrange(Start_X + houseSizeX + 1,
                                  End_X - houseSizeX - 1)
        houseZ = random.randrange(Start_Z + houseSizeZ + 1,
                                  End_Z - houseSizeZ - 1)
