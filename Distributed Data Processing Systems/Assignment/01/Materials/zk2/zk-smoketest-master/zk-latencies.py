...
parser.add_option("", "--wtest",
                  action="store_true", dest="wtest", default=False,
                  help="by default asynchronous ZK api is used, this run a single worker test")
...
def single_worker_latency_test(client, data):
    # create znode_count znodes (perm)
    start_time = time.time()
    for j in range(options.znode_count):
        client.create(child_path(j), data)
        del_cb = zkclient.DeleteCallback()
        del_cb.cv.acquire()
        client.adelete(child_path(j), del_cb)
        del_cb.waitForSuccess()
    print_elap(start_time, "Created and delete %7d permanent znodes " % (options.znode_count), options.znode_count)
...
if __name__ == '__main__':
    ...
    for i, s in enumerate(sessions):
        if options.wtest:
            print("Testing latencies on cluster %s maintaining a worker process..." % (servers[i]))
            single_worker_latency_test(s, data)
        else:
            if options.synchronous:
                print("Testing latencies on server %s using syncronous calls" % (servers[i]))
                synchronous_latency_test(s, data)
            else:
                print("Testing latencies on server %s using asyncronous calls" % (servers[i]))
