...
parser.add_option("", "--mclients",
                  dest="mclients", type="int", default=1,
...
def child_path(i, j):
    return "%s/session_%d_%d" % (options.root_znode, i, j)
...
def asynchronous_latency_test(s, data, i):
    # create znode_count znodes (perm)
    def func():
        callbacks = []
        for j in xrange(options.znode_count):
            ...
            cb.cv.acquire()
            s.acreate(child_path(i, j), cb, data)
            callbacks.append(cb)
...
if __name__ == '__main__':
    data = options.znode_size * "x" # 1-byte each
    clients_num = options.mclients
    cluster = get_zk_servers() # concatenate
    ...
    # create workers whose number is specified before, to send create requests
    for i in range(clients_num):
        server = random.choice(server) # a worker(client) randomly selects a server to be connected
        workers.append(ZKClient(server, options.timeout))
    ...
    for i, s in enumerate(workers):
        print("Testing latencies on cluster %s using asynchronous calls, %s" % (servers[i], 'worker ' + str(i)))
        asynchronous_latency_test(s, data, i)
