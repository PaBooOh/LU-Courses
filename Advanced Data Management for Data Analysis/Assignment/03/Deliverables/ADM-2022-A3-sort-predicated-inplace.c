...
void sort_predicated_inplace( int N , int *data_array ) {
    for (int i = 0; i < N-1; i++)
    {
        ...
        {
            // (1) first bit hack (bh)
            // if bh (or diff) >= 0, then bh = 0 (skip), else -1 (go into)
            // ~0 = -1 / ~-1 = 0
            int bh =  (data_array[j] - data_array[min_index]) >> 31;
            min_index = (j & bh) + (min_index & ~bh);
            // (2) second bit hack
//            int bh = (data_array[j] < data_array[min_index]); // 1/0
//            min_index = (j * bh) + (min_index * (1-bh));
            //printf("%d, %d, %d, %d\n", bh, min_index, j&bh, min_index & ~bh);
        }
        swap(&data_array[i], &data_array[min_index]);
    }
}
